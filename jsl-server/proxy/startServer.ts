...
optional arguments:
  ...
  --kill           Do not start JSL server, just kill any previously running JSL server on the specified port
                   Note that this will disrupt other windows still using the previous JSL server.
  --force          Kill any existing JSL server on the specified port, then start a new server.
                   Note that this will disrupt other windows still using the previous JSL server.
  --command name   Set which command to run for git commands (default: git)
  --cwd dir        Sets the current working directory, allowing changing the repo.
  --sl-version v   Set version number used to spawn the server (default: '(dev)')
  --platform       Set which platform implementation to use by changing the resulting URL.
                   Used to embed JSL into non-browser web environments like IDEs.
...
export function parseArgs(args: Array<string> = process.argv.slice(2)): Args {
  ...
  let kill = false;
  let force = false;
  let command = process.env.GIT_CMD ?? 'git';
